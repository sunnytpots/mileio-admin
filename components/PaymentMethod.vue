...
<!--          <small class="clear-filter clear-search trash-icon" >-->
<!--            עדכון יתרה-->
<!--          </small>-->
        </v-col>
        <v-col cols="3">
        ...
        <v-col cols="4" style="text-align: end;">
          <v-btn v-if="payments.length || (paymentMethodSelected === 'regular' && new_amount)" class="btn-primary" @click.prevent="addNewPaymentMethod" style="height: 3rem;width: 12rem;font-size: 1rem;">
            שמירה
          </v-btn>
        </v-col>
        <v-col cols="12">
    ...
    <v-container v-if="paymentMethodSelected !== 'regular'">
      <v-row>
        ...
        <v-col v-if="paymentTypeSelected === 'credit-card'" :cols="paymentTypeSelected === 'credit-card' ? '3' : '4'">
          <v-text-field
        ...
        <v-col v-if="paymentTypeSelected === 'credit-card'" :cols="paymentTypeSelected === 'credit-card' ? '4' : '4'">
          <v-text-field
...
export default {
  ...
  props: {
    ...
    currentBalance: { type: [String, Number], default: null },
  ...
  created () {
    console.log(Math.abs(this.currentBalance), 'currentBalance:::')
    console.log(this.paymentMethod, 'this.paymentMethod:::')
    this.new_amount = Math.abs(this.currentBalance).toString()
  ...
  methods: {
    ...
    async addNewPaymentMethod () {
      try {
        ...
        const res = await this.$customerInvoicesPaymentsService.updatePayment(payments)
        if (res?.status) {
    ...
    async updatePaymentMethod () {
      try {
        ...
        console.log('updatePaymentMethod', e)
      }
    },
